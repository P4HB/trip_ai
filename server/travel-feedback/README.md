...
`POST /travel/api/feedback`으로 `travel-recommendation-feedback-log-v2`를 받아 전용 SQLite에 90일 보관하는 표준 라이브러리 기반 Python 서비스다. 공개 조회 API는 없다.
...
컨테이너에는 `/data` 전용 영속 볼륨을 연결한다. 서비스는 요청 payload, 제출 UUID와 시각만 DB에 보관하고 IP·User-Agent·쿠키는 저장하지 않는다.
