...
## 날짜 기반 동선 검증 — SPEC-083

동선 검증기는 실제 모델 호출 없이 고정 운영정보·리뷰·경로·제안으로 검사한다. 알려진 시간/ID/필수/식사/종료 위반을 성공 처리하는 사례는 0건이어야 한다. 키 없는 모의 테스트의 통과는 실제 모델의 품질·지연·비용·현실 이동시간 검증을 뜻하지 않는다. 상세 승인 기준과 최신 결과는 [SPEC-083](spec_083.md#구현-결과)에 기록한다.
