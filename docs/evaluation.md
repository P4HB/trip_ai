...
- 문서 상태: 목표 설계 초안, 추천 평가는 미구현
- 최종 수정일: 2026-08-10
- 활성 설계 초안: [SPEC-008](spec_008.md)

현재 자동 추천 평가 파이프라인은 없다. 추천 기능을 구현하는 SPEC은 기능 코드와 함께 재현 가능한 평가 입력, 기준선, 지표, 회귀 결과와 출시 gate를 정의해야 한다. 현행 AI 초안 라벨을 같은 엔진의 정답으로 사용하지 않는다.
