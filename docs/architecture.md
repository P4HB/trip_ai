...
- 추천 입력·결과, 여행 MBTI 질문·pair 응답·프로필과 추천 목록·일자별 일정 장소의 1~5 추천 만족도·최대 300자 자유 의견은 브라우저 메모리에 있으며 서버나 외부 서비스로 자동 전송·저장하지 않는다. 장소별 피드백은 새 추천 조건으로 결과를 비울 때 초기화되고 같은 조건의 코스 변경에서는 장소 ID 기준으로 유지된다. 같은 장소의 추천 카드와 일정 카드 입력은 한 Map을 공유하고 화면 인스턴스끼리 즉시 동기화된다. 현재 결과의 모든 고유 장소 만족도를 고르면 사용자가 명시적으로 여행 조건·MBTI 상태·추천/일정 결과·피드백을 `travel-recommendation-feedback-log-v1` JSON 파일로 다운로드할 수 있다. 이 파일 외의 영구 복사본과 Web Storage 상태는 만들지 않는다. 공유 문구는 유형 코드·이름·공개 설명만 포함한다. 날씨·실제 이동시간·가격은 현재 계산에 없다.
