...
- `map-ui/`: 동행자·날짜·여행 방식·취향·확인을 한 단계씩 진행하는 베타 입력 흐름과 검색·지도 탐색을 제공한다. 취향 단계에서는 프리셋·세부 조정과 함께 여행 MBTI를 시작하고 결과 가중치를 적용할 수 있다. 최종 확인 뒤 `ccu-mmr-v6-travel-mbti-three-axis` 구조화 입력, P/A/M 관련도, 관련도 상위 3개를 각각 seed로 한 코스 3안, 최초 `0.5/0.3/0.2` 선택, 세션 재추천, 요청 인지형 MMR, 선택 코스 자동 일정 중심과 일차 hover/focus 지도 강조를 계산한다. 베타 추천 카드는 장소 요약·수치 없는 추천 이유·상세보기·1~5 만족도만 먼저 표시하고, ID·출처·점수 trace·41축은 장소 상세 패널에서 확인한다. `preference-elicitation.js`는 A/R·O/I·L/H 축마다 6개씩 총 18개 질문과 최대 3개 적응형 가상 pair에서 연속 원자 라벨 가중치와 3축 8개 여행 MBTI를 결정적으로 만들며 DOM과 분리된다.
...
- 추천 입력·결과, 여행 MBTI 질문·pair 응답·프로필과 장소별 1~5 추천 만족도·최대 300자 자유 의견은 브라우저 메모리에만 있으며 서버나 외부 서비스로 전송·저장하지 않는다. 장소별 피드백은 새 추천 조건으로 결과를 비울 때 초기화되고 같은 조건의 코스 변경에서는 장소 ID 기준으로 유지된다. 공유 문구는 유형 코드·이름·공개 설명만 포함한다. 날씨·실제 이동시간·가격은 현재 계산에 없다.
