...
프레임워크 없이 실행되는 정적 HTML 추천 대시보드입니다. 한국관광공사 TourAPI 제주 장소 2,154건 중 정상 좌표 2,153건을 표시하며, 24개 취향 라벨과 Companion 5개·Month 12개가 모두 있는 1,663곳을 `ccu-mmr-v0-demo`로 추천합니다. Top-N 카드와 상세에는 기존 v5 웹 조사에서 가져온 활동 설명과 원래 출처를 함께 표시합니다.
...
이 화면은 `ai_draft` 내부 비교용입니다. 웹 조사 문장도 AI 초안이므로 운영시간·가격·휴무·행사 일정은 연결된 원문에서 다시 확인해야 합니다. 날씨·영업·예약·접근성·이동시간·가격을 보장하지 않으며 사용자 입력은 브라우저 메모리에만 남습니다.
